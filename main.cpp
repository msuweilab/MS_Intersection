...
#define BLOCK_SIZE 127 //grid block-1 
...
void process_molecular_surface_newer(double probe_radius,double grid_size,double bounding_residual,char *filename_xyzr){
  //basic info
  basic_info bi;
...
#pragma omp critical
	  ...
	  //for (int l = 0; l<m_intersection_points.size(); l++)
	  //{
	  //	//map index of intersction info
	  //	m_intersection_interior[l]->grid_x += a*BLOCK_SIZE;
	  //	m_intersection_interior[l]->grid_y += b*BLOCK_SIZE;
	  //	m_intersection_interior[l]->grid_z += c*BLOCK_SIZE;
		
	  //	m_intersection_outside[l]->grid_x += a*BLOCK_SIZE;
	  //	m_intersection_outside[l]->grid_y += b*BLOCK_SIZE;
	  //	m_intersection_outside[l]->grid_z += c*BLOCK_SIZE;
	  //}
	  //cout << "map intersection index complete" << endl;
	  
#pragma omp critical
	  {
    cout<<"output intersection..."<<endl;
    int block_size=BLOCK_SIZE;
    ms.output_intersection(first_write,a,b,c,block_size);
  }	
          /*int for_amino = 0;
	    if (for_amino == 1)
...
int main(int argc, char *argv[]){
  ...
  process_molecular_surface_newer(atof(argv[2]),atof(argv[3]),atof(argv[4]),argv[1]);
  //mesh.process_molecular_surface_newer(1.4, 0.1, 1.0, "3jb9.xyzr");
  clock_t t2 = clock();
  std::cout<<"running time: "<<((float)t2-(float)t1)/CLOCKS_PER_SEC<<" seconds..."<<std::endl;
